...
//fmt.Printf("%v\n",len(ulidKey.Bytes()))
        ...
        if secondsLifetime > 0 {
                _, err = c.Do("SETEX", ulidKey.Bytes(), secondsLifetime, value)
        } else {
                _, err = c.Do("SET", ulidKey.Bytes(), value)
        }
...
func (r *Service) Get(key string) (interface{}, error) {
        ...
        ulidKey, _ := ulid.Parse(key)
        redisVal, err := c.Do("GET", ulidKey.Bytes())
...
func (r *Service) TTL(key string) (seconds int64, hasExpiration bool, found bool) {
        ...
        defer c.Close()
        ulidKey, _ := ulid.Parse(key)
        redisVal, err := c.Do("TTL", ulidKey.Bytes())
        if err != nil {
...
func (r *Service) UpdateTTL(key string, newSecondsLifeTime int64) error {
        ...
        ulidKey, _ := ulid.Parse(key)
        return r.updateTTLConn(c, ulidKey.Bytes(), newSecondsLifeTime)
}
...
func (r *Service) UpdateTTLMany(prefix string, newSecondsLifeTime int64) error {
        ...
        for _, key := range keys {
                if err = r.updateTTLConn(c, ulidKey.Bytes(), newSecondsLifeTime); err != nil { // fail on first error.
                        return err
                }
        }
